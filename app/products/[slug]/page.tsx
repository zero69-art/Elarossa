...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="min-h-screen">
    <nav className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-5 sm:px-6 sm:py-7" aria-label="Product navigation">
      ...
      <div className="flex items-center gap-4 text-[10px] tracking-[.18em] sm:gap-5 sm:text-xs"><Link href="/products" className="inline-flex min-h-11 items-center transition-opacity hover:opacity-60">SHOP</Link><Link href="/cart" className="inline-flex min-h-11 items-center transition-opacity hover:opacity-60">BAG<CartBadge /></Link></div>
    </nav>
    <div className="mx-auto max-w-7xl px-4 pb-16 sm:px-6 sm:pb-20 md:px-8">
      ...
      <section className="mt-5 grid gap-8 md:mt-8 md:grid-cols-[minmax(0,1.15fr)_minmax(340px,.85fr)] md:gap-12 lg:gap-16">
        ...
        <div className="md:sticky md:top-6 md:h-fit">
          ...
          <div className="mt-5 flex flex-wrap gap-2 text-[9px] tracking-[.16em]"><span className="rounded-full border border-[#d8ccc5] px-3 py-2">CURATED ITEM</span><span className="rounded-full border border-[#d8ccc5] px-3 py-2">{reviewLabel}</span></div>
          ...
          <div className="mt-7 grid grid-cols-3 border-y border-[#e8ded8] py-5 text-center"><div><p className="text-[9px] tracking-[.15em] opacity-50">SHIPPING</p><p className="mt-1 text-[11px]">5–12 BUSINESS DAYS</p></div><div className="border-x border-[#e8ded8]"><p className="text-[9px] tracking-[.15em] opacity-50">PACKING</p><p className="mt-1 text-[11px]">CAREFULLY</p></div><div><p className="text-[9px] tracking-[.15em] opacity-50">SUPPORT</p><p className="mt-1 text-[11px]">HERE TO HELP</p></div></div>
          <div className="mt-7 rounded-2xl bg-white/45 p-5 text-[10px] leading-5 opacity-70"><strong className="text-[11px] opacity-100">QUALITY REVIEW</strong><br />{product.qualityStatus === "approved" ? "This item has passed the current sample-review gate for sale." : "Supplier sample verification is required before this item is approved for sale. Product claims are kept conservative until that review is complete."}</div>
        </div>
      </section>
    </div>
  </main>;
