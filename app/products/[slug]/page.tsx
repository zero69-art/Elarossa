...
async function resolveProduct(slug: string) {
  ...
  try {
    ...
    return toCJStoreProduct({
      ...row,
      pid: String(row.pid ?? pid),
      productNameEn: String(row.productNameEn ?? row.nameEn ?? row.productName ?? "CJ Product"),
      bigImage: row.bigImage ?? row.productImage,
      productImageSet: row.productImageSet ?? [],
      variants: row.variants ?? response?.data?.variants ?? [],
    });
  } catch {
    return undefined;
  }
    ...
    <main className="min-h-screen">
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-6 pb-20 md:grid-cols-2">
        <div className="grid gap-4 sm:grid-cols-2">
          {product.gallery.slice(0, 4).map((image, index) => <div key={`${image}-${index}`} className="overflow-hidden rounded-3xl bg-[#e9dfda] sm:first:col-span-2"><img src={image} alt={`${product.name} ${index + 1}`} className="h-full min-h-[320px] w-full object-cover" /></div>)}
        </div>
