...
const collections = [
  ...
  { title: "Intimates", text: "Second-skin layers designed for everyday confidence.", href: "/products?q=intimates", image: "https://images.unsplash.com/photo-1596755389378-c31d21fd1273?auto=format&fit=crop&w=900&q=85" },
];
...
export default function Home() {
  ...
  return <main>
    ...
    <nav className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-5 sm:px-6 sm:py-7">
      <Link href="/products" className="text-[10px] tracking-[.25em] transition-opacity hover:opacity-60 sm:text-xs">SHOP</Link>
      <Link href="/" className="serif text-xl tracking-[.12em] sm:text-3xl">ELAROSSA</Link>
      <div className="flex items-center gap-3 text-[10px] tracking-[.16em] sm:gap-5 sm:text-xs sm:tracking-[.2em]"><Link href="/products" className="hidden transition-opacity hover:opacity-60 sm:inline">DISCOVER</Link><Link href="/cart" className="transition-opacity hover:opacity-60">BAG<CartBadge /></Link></div>
    </nav>
    ...
    <section className="bg-[#201b1b] px-5 py-16 text-center text-white sm:px-6 sm:py-24"><p className="text-[10px] tracking-[.28em] opacity-60">OUR STANDARD</p><h2 className="serif mx-auto mt-4 max-w-3xl text-3xl leading-tight sm:text-5xl md:text-6xl">Less noise. Better pieces. More confidence.</h2><p className="mx-auto mt-5 max-w-xl text-sm leading-6 opacity-65">Elarossa is built around a simple idea: discover fewer pieces, choose carefully, and improve the collection based on real customer response.</p><Link href="/products" className="mt-7 inline-block border border-white/40 px-7 py-4 text-[10px] tracking-[.2em] transition-colors hover:bg-white hover:text-[#201b1b] sm:text-xs">EXPLORE COLLECTION</Link></section>

    <footer className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-10 text-[10px] sm:px-6 sm:py-12 sm:text-xs md:flex-row md:items-center md:justify-between md:px-8"><Link href="/" className="serif text-xl tracking-widest sm:text-2xl">ELAROSSA</Link><div className="flex flex-wrap gap-x-4 gap-y-3"><Link href="/privacy" className="transition-opacity hover:opacity-60">PRIVACY</Link><Link href="/terms" className="transition-opacity hover:opacity-60">TERMS</Link><Link href="/shipping" className="transition-opacity hover:opacity-60">SHIPPING</Link><Link href="/returns" className="transition-opacity hover:opacity-60">RETURNS</Link></div><span>© 2026 Elarossa</span></footer>
  </main>;
}
