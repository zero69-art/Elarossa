...
export default function AddToBag({ product, storeLive = false }: Props) {
  ...
  return <div className="mt-7 sm:mt-8">
    <div><p className="text-[10px] tracking-[.2em]">SIZE</p><div className="mt-3 flex flex-wrap gap-2">{sizes.map(x => <button type="button" key={x} aria-pressed={size === x} onClick={() => setSize(x)} disabled={!purchasable} className={`min-h-11 border px-4 py-3 text-[10px] sm:px-5 sm:text-xs disabled:cursor-not-allowed disabled:opacity-45 ${size === x ? "border-[#201b1b] bg-[#201b1b] text-white" : "border-[#d8ccc5]"}`}>{x}</button>)}</div></div>
    <div className="mt-6"><p className="text-[10px] tracking-[.2em]">COLOR</p><div className="mt-3 flex flex-wrap gap-2">{colors.map(x => <button type="button" key={x} aria-pressed={color === x} onClick={() => setColor(x)} disabled={!purchasable} className={`min-h-11 border px-4 py-3 text-[10px] sm:px-5 sm:text-xs disabled:cursor-not-allowed disabled:opacity-45 ${color === x ? "border-[#201b1b] bg-[#201b1b] text-white" : "border-[#d8ccc5]"}`}>{x}</button>)}</div></div>
